...
namespace SerialLedWriter
{
    public class LedWriter
    {
        ...
        public void Start(String port)
        {
            ...
            if (WriteThread != null)
                throw new Exception("restarting the output is not yet supported.");
        ...
        byte[] GenerateByteArray( bool lower_chains )
        {
            ...
            foreach (var result in results)
            {
                ...
                {
                    output[(i * 9) + (result.Key % 3) + 1] = result.Value[i].Item2;
                    output[(i * 9) + (result.Key % 3) + 3 + 1] = result.Value[i].Item1;
